...
def log_regression(z,
    ...
    num_hidden = z.size(1)
    y = dataset.y.view(-1).to(test_device)
    ...
    for i in range(5):
        ...
        for epoch in range(num_epochs):
            ...
            output = classifier(z[split['train'][:, i]])
            loss = nll_loss(f(output), y[split['train'][:, i]])
            ...
            if (epoch + 1) % 20 == 0:
                if 'val' in split:
                    ...
                    test_acc = evaluator.eval({
                        'y_true': y[split['test'][:, i]].view(-1, 1),
                        'y_pred': classifier(z[split['test'][:, i]]).argmax(-1).view(-1, 1)
                    })['acc']
                    val_acc = evaluator.eval({
                        'y_true': y[split['val'][:, i]].view(-1, 1),
                        'y_pred': classifier(z[split['val'][:, i]]).argmax(-1).view(-1, 1)
                    })['acc']
