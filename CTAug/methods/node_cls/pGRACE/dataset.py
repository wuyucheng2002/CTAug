...
def get_dataset_no_norm(path, name):
    assert name in ['Cora', 'CiteSeer', 'PubMed', 'DBLP', 'Karate', 'WikiCS', 'Coauthor-CS', 'Coauthor-Phy',
                    'Amazon-Computers', 'Amazon-Photo', 'ogbn-arxiv', 'ogbg-code']
    name = 'dblp' if name == 'DBLP' else name
    # root_path = osp.expanduser('~/datasets')
    ...
    if name == 'Coauthor-Phy':
        return Coauthor(root=path, name='physics')
    ...
    # if name.startswith('ogbn'):
    #     return PygNodePropPredDataset(root=osp.join(path, 'OGB'), name=name)

    return (CitationFull if name == 'dblp' else Planetoid)(path, name)
