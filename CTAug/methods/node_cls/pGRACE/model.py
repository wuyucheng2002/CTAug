...
class Encoder_OGSN(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, activation, base_model=GCNConv,
                 k: int = 2, skip=False, id_dim=3):
        super(Encoder_OGSN, self).__init__()
        self.base_model = base_model
        self.id_dim = id_dim
        ...
        if not self.skip:
            self.conv = [base_model(in_channels, 2 * out_channels).jittable()]
            for _ in range(1, k - 1):
                self.conv.append(base_model(2 * out_channels + id_dim, 2 * out_channels))
            self.conv.append(base_model(2 * out_channels + id_dim, out_channels))
            self.conv = nn.ModuleList(self.conv)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor):
        if not self.skip:
            ...
            for i in range(self.k):
                ...
                z = self.activation(self.conv[i](z, edge_index))
            return z
        else:
            h = self.activation(self.conv[0](x, edge_index))
            hs = [self.fc_skip(x), h]
