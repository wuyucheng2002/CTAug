...
def get_core_weights(data, core, factor, device):
    weights = torch.zeros(data.x.shape[0], dtype=torch.float, device=device)
    ...
    if data.edge_index.shape[1] != 0:
        ...
        if core == 'kcore':
            for d in [0, 1, 2]:
                H = nx.k_core(G, max(core_order - d, 0))
    ...
    weights = (1 - factor * weights).to(device)
