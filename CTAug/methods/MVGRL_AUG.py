...
def MVGRL_AUG(ts, args, path, logger, dataset1, ogsn):
    ...
    for t in range(1, ts + 1):
        ...
        adj1, diff1, max_nodes = reweight_graph(graphs, factor, 0, (1 - factor), device, cal_weight)
        adj2, diff2, max_nodes = reweight_graph(graphs, 0, factor, (1 - factor), device, cal_weight)
        ...
        if ogsn:
            model1 = Model_GSN(id_dim, ft_size, hid_units, num_layers, device).to(device)
            model2 = Model_GSN(id_dim, ft_size, hid_units, num_layers, device).to(device)
        else:
