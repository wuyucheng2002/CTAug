...
def JOAOv2(ts, args, path, logger, dataset1):
    ...
    dataloader_train = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)
    ...
    for t in range(1, ts + 1):
        ...
        for epoch in range(1, epo + 1):
            loss = train(gconv, contrast_model, dataloader_train, optimizer, device, True, aug_p, pn, 0)
            losses.append(loss)
            ...
            if epoch != epo:
                ...
                for n in range(3):
                    _aug_p = np.zeros(3)
                    _aug_p[n] = 1
                    loss_aug[n] = train(gconv, contrast_model, dataloader_train, optimizer, device, False,
                                        _aug_p, pn, n+1)
